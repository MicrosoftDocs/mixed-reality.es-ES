...
Para conectarse desde la aplicación del reproductor al host, llame al método ```Connect``` en el contexto del reproductor y especifique el nombre de host y el puerto. El puerto predeterminado es **8265**.
...
1) OnConnection: se desencadena cuando se ha establecido correctamente una conexión con el host.
```cpp
