...
<td> Conocimiento de escenas</td><td style="text-align: center;">️</td><td style="text-align: center;"> ✔️</td><td style="text-align: center;"></td>
</tr>
</table>
...
Muchos de los escenarios principales para las aplicaciones compatibles con el entorno (selección de ubicación, oclusión, física etc.) son direccionables por la asignación espacial y la comprensión de escenas, en esta sección se resaltan estas diferencias. Una diferencia principal entre la comprensión de escenas y la asignación espacial es un equilibrio entre la precisión máxima y la latencia y la estructura y simplicidad. Si su aplicación requiere la menor latencia posible y requiere triángulos de malla solo, querrá acceder directamente a la asignación espacial; sin embargo, si va a realizar un procesamiento de nivel superior, puede considerar la posibilidad de cambiar al modelo de comprensión de la escena como debe proporcionar un superconjunto de funcionalidad. Tenga en cuenta también que, dado que la comprensión de la escena proporciona la malla de asignación espacial como parte de su representación, siempre tendrá acceso a los datos de asignación espacial más completos y precisos posibles.
...
##### <a name="1-scenequads-with-inference-disabled-capturing-placement-areas-for-scanned-regions"></a>[1] "SceneQuads con inferencia deshabilitada, capturando áreas de colocación para regiones digitalizadas".
...
##### <a name="2-quads-with-inference-enabled-placement-is-no-longer-limited-to-scanned-areas"></a>[2] "cuádruples con inferencia habilitada, la selección de ubicación ya no está limitada a las áreas examinadas".

Si la aplicación intenta colocar hologramas 2D o 3D en estructuras rígidas de su entorno, la simplicidad y la comodidad de SceneQuads para la selección de ubicación son preferibles para calcular esta información desde la malla de superficie. Para obtener más información sobre este tema, consulte la [Referencia del SDK](scene-understanding-SDK.md) de la escena.

**Nota:** En el caso de código heredado que depende de la malla de superficie, se puede calcular una escena que contenga la salida de la asignación espacial junto con SceneQuads, lo que garantiza que se puedan mantener los requisitos heredados para la selección de ubicación. Si la escena que comprende los datos de malla no satisface los requisitos de latencia de la aplicación, se recomienda seguir usando las API de asignación espacial que se documentan aquí: [Colocación de asignación espacial](spatial-mapping.md#placement)

### <a name="occlusion"></a>Oclusión

La [oclusión de asignación espacial](spatial-mapping.md#occlusion) sigue siendo la manera menos latente de capturar el estado en tiempo real del entorno. Aunque esto puede ser útil para proporcionar la oclusión en escenas muy dinámicas, es posible que desee considerar la posibilidad de realizar una comprensión de la escena por varias razones. Si usa la malla de asignación espacial generada por la comprensión de la escena, puede solicitar datos de la asignación espacial que no se almacenarán en la memoria caché local y, por lo tanto, no se disponible de las API de percepción. El uso de la asignación espacial para la oclusión junto con las mallas estancas proporcionará un valor adicional, la finalización específica de la estructura de sala sin explorar.

Si sus requisitos pueden tolerar el aumento de la latencia de la comprensión de la escena, los desarrolladores de aplicaciones deben considerar la posibilidad de usar la escena para comprender la malla estanca y, presumiblemente, la malla de asignación espacial al unísono con representaciones planas. Esto proporcionaría "lo mejor de ambos mundos", en el que la oclusión estanca simplificada se casado con geometría no plana más precisa que proporciona los mapas de oclusión más realistas posibles.

### <a name="physics"></a>Efectos

La comprensión de escenas genera mallas estancos que descomponen espacio con semántica específica para abordar muchas limitaciones en la física que imponen las mallas de superficie. Las estructuras estancas garantizan que siempre se alcanzan las conversiones de rayos físicos y la descomposición semántica permite una generación más sencilla de mallas de navegación para la navegación en interiores. Tal y como se describe en la sección [olcusión](#occlusion) sobre la creación de una escena con EnableSceneObjectMeshes y EnableWorldMesh, se producirá la malla completa más física posible. La propiedad estanca de la malla del entorno impedirá que las pruebas de posicionamiento no se realicen en las superficies de posicionamiento y los datos de la malla garantizarán que la física interactúe con todos los objetos de la escena, y no solo con la estructura de la habitación.
